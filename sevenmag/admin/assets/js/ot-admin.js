...
!function ($) {
  ...
  $(document).on('ready', function () {
	jQuery('#page_template').change(function(){
		if ( jQuery('#page_template').val() === 'page-home-slideshow.php' ) {
			jQuery('#slideshow-options').show();
		} else {
			jQuery('#slideshow-options').hide();
		}  
	});
	if ( jQuery('#page_template').val() === 'page-home-slideshow.php' ) {
		jQuery('#slideshow-options').show();
	...
	jQuery('#_loop-0, #_loop-1').click(function() {
		if(jQuery('#_loop-0').is(':checked')) { 
			jQuery('#setting__loop_category').fadeIn(); 
			jQuery('#setting_masonry_meta').fadeIn(); 
		} else {
			jQuery('#setting__loop_category').fadeOut(); 
			jQuery('#setting_masonry_meta').fadeOut(); 
			jQuery('#setting_masonry_col').fadeOut(); 
		}
	});
	if(jQuery('#_loop-1').is(':checked')) { 
		jQuery('#setting__loop_category').hide();
		jQuery('#setting_masonry_meta').hide(); 
	}

	jQuery('#masonry_meta-1, #masonry_meta-0').click(function() {
		if(jQuery('#masonry_meta-0').is(':checked')) { 
			jQuery('#setting_masonry_col').fadeIn(); 
		} else {
			jQuery('#setting_masonry_col').fadeOut(); 
		}
	});
	if(jQuery('#masonry_meta-1').is(':checked')) { 
		jQuery('#setting_masonry_col').hide(); 
	}
    
	jQuery('#page_template').change(function(){
		if ( jQuery('#page_template').val() === 'page-archive.php' ) {
			jQuery('#setting__page_comments, #setting__loop').hide();
		} else {
			jQuery('#setting__page_comments, #setting__loop').show();
		}  
	});
	if ( jQuery('#page_template').val() === 'page-archive.php' ) {
		jQuery('#setting__page_comments, #setting__loop').hide();
